...
function PlayerForScout() {
  ...
  const getNextMatch = async () => {
    ...
    const nextMatch = responses.data.filter(
      (match: { date: string | number | Date }) => new Date(match.date).getTime() > today,
    );
    ...
    <>
      ...
      <div className="player_infos_footer">
        ...
        <div className="player_infos_footer_content">
          <p>
            Né le:
            {new Date(infos?.birth_date as unknown as Date).toLocaleDateString("fr-FR", { year: "numeric", month: "long", day: "numeric"})}
          </p>
          <p>
            Genre:
            {infos?.genre}
